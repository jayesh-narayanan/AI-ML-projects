...
def load_all_images_from_folder(base_dir, img_size=(32, 32)):
    ...
    # Convert to numpy arrays for ease of operation
    ...
    print(f"Features per image: {X.shape[1]} (32x32x3)")
...
if __name__ == "__main__":
    ...
    for hidden_arch in hidden_archs:
        ...
        # Compute time to compare models
    ...
    # Plot Accuracy vs hidden layer units
    
    # # Plot 1: Accuracy
    # plt.figure(figsize=(12, 8))
    # plt.plot(hidden_layer_units, train_accuracies, marker='o', label='Training Accuracy', 
    #          linewidth=2, markersize=8)
    # plt.plot(hidden_layer_units, test_accuracies, marker='s', label='Testing Accuracy', 
    #          linewidth=2, markersize=8)
    # plt.xlabel('Number of hidden layer units', fontsize=12)
    # plt.ylabel('Accuracy', fontsize=12)
    # plt.title('Accuracy vs hidden layer units', fontsize=14)
    # plt.legend(fontsize=11)
    # plt.grid(True, alpha=0.3)
    # plt.tight_layout()
    # plt.savefig('accuracy_vs_hidden_units.png', dpi=300, bbox_inches='tight')
    # plt.show()
    
    # Plot 2: F1 Score
    plt.figure(figsize=(12, 8))
    plt.plot(hidden_layer_units, train_f1_scores, marker='o', label='Training F1', 
             linewidth=2, markersize=8)
    plt.plot(hidden_layer_units, test_f1_scores, marker='s', label='Testing F1', 
             linewidth=2, markersize=8)
    plt.xlabel('Number of hidden layer units', fontsize=12)
    plt.ylabel('Avg F1 score', fontsize=12)
    plt.title('Avg F1 score vs hidden layer units', fontsize=14)
    plt.legend(fontsize=11)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('f1_scores_vs_hidden_units.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    for units, train_acc, test_acc, train_f1, test_f1 in zip(
        hidden_layer_units, train_accuracies, test_accuracies, train_f1_scores, test_f1_scores
    ):
