...
for depth in depths:
    ...
    tree.post_prune(val_df)
    number_of_nodes.append(tree.number_of_nodes(tree.root))
    ...
    # only save the prediction of the tree which had max_depth of 35
    if depth==35:
        test_predictions=tree.predict(test_df)


# Plot Accuracy agaist nodes
plt.figure(figsize=(12, 8))
plt.plot(number_of_nodes, train_accuracies, marker='o', label='Training Accuracy', 
            linewidth=2, markersize=8)
plt.plot(number_of_nodes, test_accuracies, marker='s', label='Testing Accuracy', 
            linewidth=2, markersize=8)
plt.plot(number_of_nodes, val_accuracies, marker='^', label='Validation Accuracy', 
            linewidth=2, markersize=8)
plt.xlabel('Number of nodes', fontsize=12)
plt.ylabel('Accuracy', fontsize=12)
plt.title('Accuracy vs Nodes', fontsize=14)
plt.legend(fontsize=11)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('accuracy_vs_nodes_c.png', dpi=300, bbox_inches='tight')
plt.show()

# save the predictions into a csv file
predictions_df = pd.DataFrame(test_predictions, columns=['result'])
output_dir = r"C:\IITD\Sem-5\COL774\Assignment 3\outputs"
os.makedirs(output_dir, exist_ok=True)
predictions_df.to_csv(os.path.join(output_dir, 'predictions_c.csv'), header=True, index=False)
